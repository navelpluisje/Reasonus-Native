...
ReaSonusPluginMappingConverter::ReaSonusPluginMappingConverter()
    ...
    select = std::regex(
        "Select([0-9]{1,2})[[:space:]]+FXParam[[:space:]]+([0-9]{1,6})[[:space:]]+\"([a-zA-Z0-9[:space:]]+)\"([[:space:]]+[[][[:space:]]([0-9.[:space:]]+)[[:space:]])?");
    fader = std::regex(
        "Fader([0-9]{1,2})[[:space:]]+FXParam[[:space:]]+([0-9]{1,6})[[:space:]]+\"([a-zA-Z0-9[:space:]]+)\"");
    scribble = std::regex(
        "ScribbleLine([0-9])_([0-9])[[:space:]]+FXParamNameDisplay[[:space:]]+([0-9]{1,6})[[:space:]]+\"([a-zA-Z0-9[:space:]]+)\"");
...
void ReaSonusPluginMappingConverter::SetBaseFileNames(const std::string &folder) {
    file_names.clear();
    int index = 0;
...
void ReaSonusPluginMappingConverter::ConvertFile(const std::string &file_path) {
    ...
    for (std::string line; getline(zon_file, line);) {
        if (line.rfind("Zone ", 0) == 0) {
            ...
            if (std::regex_search(line, matches, fader)) {
                const std::string key = "fader_" + std::to_string(stoi(matches[1].str()) - 1);
            ...
            if (std::regex_search(line, matches, select)) {
                const std::string key = "select_" + std::to_string(stoi(matches[1].str()) - 1);
                const std::string steps = matches[5].str() == ""
                                              ? "2"
                                              : std::to_string(split(matches[5].str(), " ").size());
        ...
        if (ImGui::BeginChild(m_ctx, "actions_container", 0.0, -148.0, ImGui::ChildFlags_None)) {
            if (ImGui::BeginChild(m_ctx, "actions_info", 300.0, 0.0, ImGui::ChildFlags_None)) {
                ...
                if (ImGui::BeginChild(m_ctx, "actions_convert_info", 0.0, 0.0, ImGui::ChildFlags_FrameStyle)) {
                    ...
                    ImGui::Text(
                        m_ctx,
                        "This action will convert your old zon plugin files to the new ini files used by ReaSonus Native. If a file already exists for ReaSonus Native, the file will be ignored.");
                    ImGui::Text(
                        m_ctx, "Once the action is finished, it will list the converted files and the skipped files.");
                    ImGui::Text(m_ctx, "The unconverted files have 2 types:");
                    ImGui::BulletText(m_ctx, "With 'zon' extension. These files were invalid");
                    ImGui::BulletText(
                        m_ctx, "Without 'zon' extension. These files already exist and will not be overwritten");
                ...
                if (ImGui::BeginChild(m_ctx, "main_content_area", 0.0, 0.0, ImGui::ChildFlags_FrameStyle)) {
                    ...
                    if (ImGui::BeginListBox(m_ctx, "##converted_files", 0.0, 110)) {
                        ...
                        for (const auto &converted_file: converted_files) {
                            ImGui::Selectable(m_ctx, converted_file.c_str(), &selected,
                    ...
                    ReaSonusPageTitle(m_ctx, assets, "Unconverted files", false);

                    UiStyledElements::PushReaSonusListBoxStyle(m_ctx);
                    if (ImGui::BeginListBox(m_ctx, "##unconverted_files", 0.0, 110)) {
                        bool selected = false;
                        for (const auto &unconverted_file: unconverted_files) {
                            ImGui::Selectable(m_ctx, unconverted_file.c_str(), &selected,
                                              ImGui::SelectableFlags_AllowOverlap);
                        }
