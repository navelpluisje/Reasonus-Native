    ...
    const std::string *message)
{
    ...
    if (ImGui::BeginChild(m_ctx, "button-bar", 0.0, 36.0, ImGui::ChildFlags_FrameStyle))
    {
        ...
        const float buttonWidth1 = static_cast<float>(x_pos_1) + button_padding_x * 2.F;
        ImGui::CalcTextSize(m_ctx, cancel_label.c_str(), &x_pos_1, &y_pos_1);

        const float buttonWidth2 = static_cast<float>(x_pos_1) + button_padding_x * 2.F;
        ImGui::GetStyleVar(m_ctx, ImGui::StyleVar_ItemSpacing, &x_pos_2, &y_pos_2);

        const float widthNeeded = buttonWidth1 + x_pos_2 + buttonWidth2;
