...
static void ReaSonusGoToInput(ImGui_Context *m_ctx, CSurf_UI_Assets *assets, int *value, const int max_value) {
    double modal_x;
    double modal_y;
    ImGui::GetCursorScreenPos(m_ctx, &modal_x, &modal_y);

    static char str_value[255]; // NOLINT(*-avoid-c-arrays)
    strcpy(str_value, std::to_string(*value + 1).c_str()); // NOLINT(*-security.insecureAPI.strcpy)

    UiStyledElements::PushReaSonusFieldGroupStyle(m_ctx);
    if (ImGui::BeginChild(m_ctx, "container-goto-input", 30.0, 0.0,
                          ImGui::ChildFlags_FrameStyle | ImGui::ChildFlags_AutoResizeY)) {
        const bool is_active = ImGui::IsPopupOpen(m_ctx, "goto-modal");
        ...
        if (ImGui::BeginPopup(m_ctx, "goto-modal",
            ...
            if (ImGui::BeginChild(m_ctx, "goto-modal-content", 98, 46, ImGui::ChildFlags_FrameStyle,
                                  ImGui::WindowFlags_NoNavFocus | ImGui::WindowFlags_NoScrollbar)) {
                ...
                if (ImGui::InputText(
                    ...
                    assets->GetOnlyDigitsFilter())) {
                    if (!std::string(str_value).empty() && isInteger(std::string(str_value))) {
                        const int val = stoi(std::string(str_value)) - 1;
                        if (val < max_value) {
                            *value = val;
                        }
                    } else {
