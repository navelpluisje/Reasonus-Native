#include "action_fp_8_setting_disable_plugins.hpp"
#include <mini/ini.h>
...
namespace ACTION_FP_8_SETTING_DISABLE_PLUGINS {
    // some global non-const variables
    // the necessary 'evil'
    int command_id{0};
    bool toggle_action_state{false};
    constexpr auto command_name = "REASONUS_FP8_TOGGLE_DISABLE_PLUGINS";
    ...
    std::string ReadIniValue(const std::string &group, const std::string &item) {
        return ReaSonusSettings::GetInstance(FP_8)->GetSetting(group, item);
    }

    void WriteIniValue(const std::string &group, const std::string &item, const std::string &value) {
