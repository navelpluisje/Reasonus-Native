#include "action_fp_8_setting_distraction_free_mode.hpp"
#include <mini/ini.h>
...
namespace ACTION_FP_8_SETTING_DISTRACTION_FREE_MODE
{
    ...
    constexpr auto command_name = "REASONUS_FP8_TOGGLE_DISTRACTION_FREE_MODE";
    constexpr auto action_name = "Reasonus: Toggle distraction free setting";
    ...
    void MainFunctionOfMyPlugin()
    {
        ...
        {
            WriteIniValue("surface", "distraction-free", "0");
        }
        else
        {
            WriteIniValue("surface", "distraction-free", "1");
        }
    }
    ...
    auto ToggleActionCallback(int command) -> int
    {
        ...
        if (stoi(ReadIniValue("surface", "distraction-free")) == 1) // if toggle_action_state == true
        {
            return 1;
        }
        return 0;
