#include "action_fp_v2_setting_control_hidden_tracks.hpp"
#include <mini/ini.h>
...
namespace ACTION_FP_V2_SETTING_CONTROL_HIDDEN_TRACKS {
    ...
    constexpr auto command_name = "REASONUS_FPV2_TOGGLE_CONTROL_HIDDEN_TRACKS";
    constexpr auto action_name = "Reasonus: v2 Toggle control hidden tracks";
    ...
    std::string ReadIniValue(std::string group, std::string item) {
        return ReaSonusSettings::GetInstance(FP_V2)->GetSetting(group, item);
    }

    void WriteIniValue(std::string group, std::string item, std::string value) {
        ReaSonusSettings::GetInstance(FP_V2)->SetAndSaveSetting(group, item, value);
    }
    ...
    void MainFunctionOfMyPlugin() {
        if (toggle_action_state) {
            WriteIniValue("surface", "control-hidden-tracks", "0");
        } else {
            WriteIniValue("surface", "control-hidden-tracks", "1");
        }
    }
    ...
    auto ToggleActionCallback(int command) -> int {
        ...
        if (stoi(ReadIniValue("surface", "control-hidden-tracks")) == 1) // if toggle_action_state == true
        {
            return 1;
        }
        return 0;
