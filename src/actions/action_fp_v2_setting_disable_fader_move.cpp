#include "action_fp_v2_setting_disable_fader_move.hpp"
#include <mini/ini.h>
...
namespace ACTION_FP_V2_SETTING_DISABLE_FADER_MOVE {
    ...
    void MainFunctionOfMyPlugin() {
        if (toggle_action_state) {
            WriteIniValue("surface", "can-disable-fader", "0");
        } else {
            WriteIniValue("surface", "can-disable-fader", "1");
        }
    }
    ...
    auto ToggleActionCallback(int command) -> int {
        ...
        if (stoi(ReadIniValue("surface", "can-disable-fader")) == 1) // if toggle_action_state == true
        {
            return 1;
        }
        return 0;
