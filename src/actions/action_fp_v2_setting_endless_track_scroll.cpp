#include "action_fp_v2_setting_endless_track_scroll.hpp"
#include <mini/ini.h>
...
namespace ACTION_FP_V2_SETTING_ENDLESS_TRACK_SCROLL {
    ...
    constexpr auto command_name = "REASONUS_FPV2_TOGGLE_ENDLESS_TRACK_SCROLL";
    constexpr auto action_name = "Reasonus: v2 Toggle endless track scroll";
    ...
    void MainFunctionOfMyPlugin() {
        if (toggle_action_state) {
            WriteIniValue("surface", "endless-track-scroll", "0");
        } else {
            WriteIniValue("surface", "endless-track-scroll", "1");
        }
    }
    ...
    auto ToggleActionCallback(int command) -> int {
        ...
        if (stoi(ReadIniValue("surface", "endless-track-scroll")) == 1) // if toggle_action_state == true
        {
            return 1;
        }
        return 0;
