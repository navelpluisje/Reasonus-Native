...
protected:
    void GetFaderValue(MediaTrack *media_track, int send_index, int *fader_value, int *valuebar_value, double *_pan,
                       std::string *pan_str) {
...
public:
    ...
    void UpdateTracks(bool force_update) override {
        ...
        for (int i = 0; i < context->GetNbChannels(); i++) {
            ...
            std::string pan_str;
            GetFaderValue(sends_track, send_index, &fader_value, &valuebar_value, &pan, &pan_str);
            ...
            if (!media_track) {
                ...
                track->SetDisplayLine(0, ALIGN_LEFT, DAW::GetTrackName(media_track).c_str(),
                                      sends_track == media_track ? INVERT : NON_INVERT, force_update);
            }
            ...
            if (DAW::HasTrackSend(sends_track, send_index)) {
                if (add_send_enabled) {
                    ...
                    track->SetDisplayLine(1, ALIGN_LEFT, DAW::GetTrackSendDestName(sends_track, send_index).c_str(),
                                          INVERT, force_update);
                    track->SetDisplayLine(2, ALIGN_CENTER,
                                          DAW::GetTrackSurfaceSendMode(sends_track, send_index).c_str(), NON_INVERT,
                                          force_update);
                }
                track->SetDisplayLine(3, ALIGN_CENTER,
                                      DAW::GetTrackSurfaceSendAutoMode(sends_track, send_index).c_str(), NON_INVERT,
                                      force_update);
