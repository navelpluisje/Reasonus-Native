...
void CSurf_FP_8_FaderManager::SetChannelMode(const ChannelMode channelMode, const bool updateButtons = false) {
...
void CSurf_FP_8_FaderManager::Refresh(const bool force) {
...
void CSurf_FP_8_FaderManager::HandlePluginsButtonClick(const int value, const bool track) {
    if (value == 0) {
        return;
    }

    context->SetPluginEditPluginId(-1);
    const bool cameFromPlugin = context->IsChannelMode(PluginEditMode) || context->IsChannelMode(PluginControlMode);

    if ((!context->IsChannelMode(PluginMode) && !track && !cameFromPlugin) || (
            context->GetPreviousPluginChannelMode() == PluginMode && cameFromPlugin)) {
...
void CSurf_FP_8_FaderManager::HandleSendButtonClick(const int value, const bool track) {
    if (value == 0) {
        return;
    }
...
void CSurf_FP_8_FaderManager::HandlePanButtonClick(const int value, const bool track) {
    if (value == 0) {
        return;
    }
...
void CSurf_FP_8_FaderManager::HandleTouchButtonClick() {
    if (context->IsChannelMode(MenuMode)) {
        const ChannelMode mode = context->GetPreviousChannelMode();
        SetChannelMode(mode);
    } else {
...
void CSurf_FP_8_FaderManager::UpdateTracks(const bool force_update) const {
    if (force_update) {
...
void CSurf_FP_8_FaderManager::HandleMuteClick(const int index, const int value) const {
    channelManager->HandleMuteClick(index, value);
}

void CSurf_FP_8_FaderManager::HandleEncoderIncrement() const {
...
void CSurf_FP_8_FaderManager::HandleEncoderDecrement() const {
    if (context->IsChannelMode(MenuMode) || context->GetPanEncoderMode() == PanEncoderPluginStepSizeMode) {
        channelManager->HandleEndcoderDecrement(1);
    }
}

void CSurf_FP_8_FaderManager::HandleEncoderPush() const {
    if (context->IsChannelMode(MenuMode) || context->IsChannelMode(PluginControlMode)) {
        channelManager->HandleEndcoderPush(1);
    }
}

void CSurf_FP_8_FaderManager::HandleSoloClick(const int index, const int value) {
...
void CSurf_FP_8_FaderManager::HandleSelectClick(const int index, const int value) {
...
void CSurf_FP_8_FaderManager::HandleFaderMove(const int index, const int msb, const int lsb) const {
    channelManager->HandleFaderMove(index, msb, lsb);
}

void CSurf_FP_8_FaderManager::HandleFaderTouch(const int index, const int value) const {
