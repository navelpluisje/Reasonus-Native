...
protected:
    ...
    void GetFaderValue(MediaTrack *media_track, int send_index, int *fader_value, int *valuebar_value, int *_pan,
                       std::string *pan_str) {
...
public:
    ...
    void UpdateTracks(bool force_update) override {
        ...
        for (int i = 0; i < context->GetNbChannels(); i++) {
            ...
            int pan, fader_value, valuebar_value = 0;
            std::string pan_str;
            ...
            GetFaderValue(media_track, send_index, &fader_value, &valuebar_value, &pan, &pan_str);
            ...
            if (DAW::HasTrackSend(media_track, send_index)) {
                if (add_send_enabled) {
                    ...
                    track->SetDisplayLine(1, ALIGN_LEFT, DAW::GetTrackSendDestName(media_track, send_index).c_str(),
                                          INVERT, force_update);
                    track->SetDisplayLine(2, ALIGN_CENTER,
                                          DAW::GetTrackSurfaceSendMode(media_track, send_index).c_str(), NON_INVERT,
                                          force_update);
                }
                track->SetDisplayLine(3, ALIGN_CENTER, Progress(send_index + 1, nb_track_sends[i]).c_str(), NON_INVERT,
                                      force_update);
